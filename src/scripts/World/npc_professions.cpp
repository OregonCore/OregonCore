    ...
    for (int i=0; i<3; i++)
    {
        ...
        for (int j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; j++)
        {
                ...
                if (pItem->GetProto()->RequiredSpell == reqSpell)
            {
                ...
                debug_log("OSCR: player attempt to unlearn spell %u, but item %u is equipped.",reqSpell,pItem->GetProto()->ItemId);
                return false;
            }
        }
    }
...
bool GossipHello_npc_prof_alchemy(Player *player, Creature* pCreature)
{
...
void SendActionMenu_npc_prof_alchemy(Player *player, Creature* pCreature, uint32 action)
{
    switch(action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            ...
            player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, pCreature, 0, 0);
        ...
        case GOSSIP_ACTION_INFO_DEF + 4:
            ...
            {
                pCreature->CastSpell(player, S_UNLEARN_TRANSMUTE, true);
        ...
        case GOSSIP_ACTION_INFO_DEF + 5:
            if (player->GetMoney() >= DoHighUnlearnCost(player))
            {
                pCreature->CastSpell(player, S_UNLEARN_ELIXIR, true);
                player->ModifyMoney(-DoHighUnlearnCost(player));
            } else
            player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, pCreature, 0, 0);
            player->CLOSE_GOSSIP_MENU();
        ...
        case GOSSIP_ACTION_INFO_DEF + 6:
            if (player->GetMoney() >= DoHighUnlearnCost(player))
            {
                pCreature->CastSpell(player, S_UNLEARN_POTION, true);
                player->ModifyMoney(-DoHighUnlearnCost(player));
            } else
            player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, pCreature, 0, 0);
            player->CLOSE_GOSSIP_MENU();
...
void SendConfirmLearn_npc_prof_alchemy(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
void SendConfirmUnlearn_npc_prof_alchemy(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
bool GossipSelect_npc_prof_alchemy(Player *player, Creature* pCreature, uint32 sender, uint32 action)
{
    switch(sender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_alchemy(player, pCreature, action); break;
        case GOSSIP_SENDER_LEARN:   SendConfirmLearn_npc_prof_alchemy(player, pCreature, action); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_alchemy(player, pCreature, action); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_alchemy(player, pCreature, action); break;
    }
    return true;
}
...
bool GossipHello_npc_prof_blacksmith(Player *player, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        player->PrepareQuestMenu(pCreature->GetGUID());
    if (pCreature->isVendor())
...
void SendActionMenu_npc_prof_blacksmith(Player *player, Creature* pCreature, uint32 action)
{
    switch(action)
    {
        case GOSSIP_ACTION_TRADE:
        ...
        case GOSSIP_ACTION_INFO_DEF + 1:
            if (!player->HasSpell(S_ARMOR))
            {
                ...
                //pCreature->CastSpell(player, S_REP_ARMOR, true);
            }
            player->CLOSE_GOSSIP_MENU();
            break;
        case GOSSIP_ACTION_INFO_DEF + 2:
            if (!player->HasSpell(S_WEAPON))
            {
                ...
                //pCreature->CastSpell(player, S_REP_WEAPON, true);
            }
            player->CLOSE_GOSSIP_MENU();
            break;
        ...
        case GOSSIP_ACTION_INFO_DEF + 3:
            if (HasWeaponSub(player))
            {
                ...
                pCreature->MonsterSay(TALK_MUST_UNLEARN_WEAPON,0,player->GetGUID());
            }
...
void SendConfirmLearn_npc_prof_blacksmith(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
void SendConfirmUnlearn_npc_prof_blacksmith(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
bool GossipSelect_npc_prof_blacksmith(Player *player, Creature* pCreature, uint32 sender, uint32 action)
{
    switch(sender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_blacksmith(player, pCreature, action); break;
        case GOSSIP_SENDER_LEARN:   SendConfirmLearn_npc_prof_blacksmith(player, pCreature, action); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_blacksmith(player, pCreature, action); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_blacksmith(player, pCreature, action); break;
    }
    return true;
}

/*bool QuestComplete_npc_prof_blacksmith(Player *player, Creature* pCreature, Quest const *_Quest)
{
    if ((_Quest->GetQuestId() == 5283) || (_Quest->GetQuestId() == 5301))             //armorsmith
        pCreature->CastSpell(player, 17451, true);

    if ((_Quest->GetQuestId() == 5284) || (_Quest->GetQuestId() == 5302))             //weaponsmith
        pCreature->CastSpell(player, 17452, true);
...
bool GossipHello_npc_prof_leather(Player *player, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        player->PrepareQuestMenu(pCreature->GetGUID());
    if (pCreature->isVendor())
...
void SendActionMenu_npc_prof_leather(Player *player, Creature* pCreature, uint32 action)
{
    switch(action)
    {
        case GOSSIP_ACTION_TRADE:
...
void SendConfirmUnlearn_npc_prof_leather(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
bool GossipSelect_npc_prof_leather(Player *player, Creature* pCreature, uint32 sender, uint32 action)
{
    switch(sender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_leather(player, pCreature, action); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_leather(player, pCreature, action); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_leather(player, pCreature, action); break;
    }
    return true;
}
...
bool GossipHello_npc_prof_tailor(Player *player, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        player->PrepareQuestMenu(pCreature->GetGUID());
    if (pCreature->isVendor())
...
void SendActionMenu_npc_prof_tailor(Player *player, Creature* pCreature, uint32 action)
{
    switch(action)
    {
        case GOSSIP_ACTION_TRADE:
...
void SendConfirmLearn_npc_prof_tailor(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
void SendConfirmUnlearn_npc_prof_tailor(Player *player, Creature* pCreature, uint32 action)
{
    if (action)
    {
        uint32 eCreature = pCreature->GetEntry();
...
bool GossipSelect_npc_prof_tailor(Player *player, Creature* pCreature, uint32 sender, uint32 action)
{
    switch(sender)
    {
        case GOSSIP_SENDER_MAIN:    SendActionMenu_npc_prof_tailor(player, pCreature, action); break;
        case GOSSIP_SENDER_LEARN:   SendConfirmLearn_npc_prof_tailor(player, pCreature, action); break;
        case GOSSIP_SENDER_UNLEARN: SendConfirmUnlearn_npc_prof_tailor(player, pCreature, action); break;
        case GOSSIP_SENDER_CHECK:   SendActionMenu_npc_prof_tailor(player, pCreature, action); break;
    }
    return true;
}

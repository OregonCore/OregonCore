...
class DestinationHolder
{
    ...
    public:
        ...
        bool UpdateExpired(void) const
        {
            return i_tracker.Passed();
        }
        void ResetUpdate(uint32 t = TRAVELLER_UPDATE_INTERVAL)
        {
            i_tracker.Reset(t);
        }
        uint32 GetTotalTravelTime(void) const
        {
            return i_totalTravelTime;
        }
        void IncreaseTravelTime(uint32 increment)
        {
            i_totalTravelTime += increment;
        }
        ...
        bool HasArrived(void) const
        {
            return (i_totalTravelTime == 0 || i_timeElapsed >= i_totalTravelTime);
        }
        ...
        void GetLocationNow(const Map* map, float& x, float& y, float& z, bool is3D = false);
        void GetLocationNowNoMicroMovement(float& x, float& y, float& z); // For use without micro movement

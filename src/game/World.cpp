...
void World::LoadConfigSettings(bool reload)
{
    ...
    if (reload)
    {
        if (dataPath!=m_dataPath)
            sLog.outError("DataDir option can't be changed at Oregond.conf reload, using current value (%s).",m_dataPath.c_str());
    }
    else
    {
        m_dataPath = dataPath;
...
void World::SetInitialWorldSettings()
{
    ...
    {
        sLog.outError("Correct *.map files not found in path '%smaps' or *.vmap/*vmdir files in '%svmaps'. Please place *.map/*.vmap/*.vmdir files in appropriate directories or correct the DataDir value in the Oregond.conf file.",m_dataPath.c_str(),m_dataPath.c_str());
        exit(1);
    }
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= m_gameTime))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            {
                ...
                CellPair p(Oregon::ComputeCellPair(summoner->GetPositionX(), summoner->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;

                Oregon::GameObjectWithDbGUIDCheck go_check(*summoner,step.script->datalong);
            ...
            case SCRIPT_COMMAND_OPEN_DOOR:
            {
                ...
                CellPair p(Oregon::ComputeCellPair(caster->GetPositionX(), caster->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;

                Oregon::GameObjectWithDbGUIDCheck go_check(*caster,step.script->datalong);
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    CellPair p(Oregon::ComputeCellPair(((Unit*)source)->GetPositionX(), ((Unit*)source)->GetPositionY()));
                    Cell cell(p);
                    cell.data.Part.reserved = ALL_DISTRICT;

                    //sLog.outDebug("Attempting to find Creature: Db GUID: %i", step.script->datalong);
                    Oregon::CreatureWithDbGUIDCheck target_check(((Unit*)source), step.script->datalong);

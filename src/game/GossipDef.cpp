...
void PlayerMenu::SendQuestGiverQuestList(QEmote eEmote, const std::string& Title, uint64 npcGUID)
{
    ...
    pSession->SendPacket(&data);
    sLog.outStaticDebug("WORLD: Sent SMSG_QUESTGIVER_QUEST_LIST NPC Guid=%u", GUID_LOPART(npcGUID));
}
...
void PlayerMenu::SendQuestGiverStatus(uint8 questStatus, uint64 npcGUID)
{
    ...
    pSession->SendPacket(&data);
    sLog.outStaticDebug("WORLD: Sent SMSG_QUESTGIVER_STATUS NPC Guid=%u, status=%u", GUID_LOPART(npcGUID), questStatus);
}
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const *pQuest, uint64 npcGUID, bool ActivateAccept)
{
    ...
    sLog.outStaticDebug("WORLD: Sent SMSG_QUESTGIVER_QUEST_DETAILS NPCGuid=%u, questid=%u", GUID_LOPART(npcGUID), pQuest->GetQuestId());
}
...
void PlayerMenu::SendQuestQueryResponse(Quest const *pQuest)
{
    ...
    pSession->SendPacket(&data);
    sLog.outStaticDebug("WORLD: Sent SMSG_QUEST_QUERY_RESPONSE questid=%u", pQuest->GetQuestId());
}

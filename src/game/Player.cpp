...
bool Player::ModifyOneFactionReputation(FactionEntry const* factionEntry, int32 standing)
{
    ...
    if (itr != m_factions.end())
    {
        ...
        else if (new_rep < Reputation_Bottom)
...
{
    QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT itemEntry, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, itemTextId, item_guid, item_template FROM mail_items WHERE mail_id='%u'", mail->messageID);
    if (!result)
        return;
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        if (LevelMin || LevelMax || missingItem || missingKey || missingQuest || missingHeroicQuest)
        {
            ...
            {
                if (missingItem)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetOregonString(LANG_LEVEL_MINREQUIRED_AND_ITEM), ar->levelMin, sObjectMgr.GetItemPrototype(missingItem)->Name1);
                else if (missingKey)

    ...
    std::string const& senderName /*= ""*/, std::string const& receiverName /*= ""*/, std::string const& channelName /*= ""*/)
{
    ...
    case CHAT_MSG_RAID_BOSS_WHISPER:
        ...
        {
            data << uint32(receiverName.c_str() + 1);
            data << receiverName.c_str();
        }
        break;
    ...
    data << message.c_str();
    if (chatTag != CHAT_MSG_REPLY && chatTag != CHAT_MSG_DND && chatTag != CHAT_MSG_AFK)
        data << uint8(chatTag);
    else
        data << uint8(0);
    ...
    std::string const& channelName /*= ""*/, LocaleConstant locale /*= DEFAULT_LOCALE*/)
{
    ...
    return BuildChatPacket(data, chatType, language, senderGUID, receiverGUID, message, chatTag, senderName, receiverName, channelName);
}

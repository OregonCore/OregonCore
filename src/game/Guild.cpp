...
bool Guild::create(Player* leader, std::string gname)
{
    ...
    CreateRank(lSession->GetOregonString(LANG_GUILD_MASTER),  GR_RIGHT_ALL);
    CreateRank(lSession->GetOregonString(LANG_GUILD_OFFICER), GR_RIGHT_ALL);
    CreateRank(lSession->GetOregonString(LANG_GUILD_VETERAN), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    CreateRank(lSession->GetOregonString(LANG_GUILD_MEMBER),  GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    CreateRank(lSession->GetOregonString(LANG_GUILD_INITIATE),GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
...
void Guild::DelRank()
{
    ...
    CharacterDatabase.PExecute("DELETE FROM guild_bank_right WHERE rid>='%u' AND guildid='%u'", rank, Id);
...
void Guild::SetBankRightsAndSlots(uint32 rankId, uint8 TabId, uint32 right, uint32 nbSlots, bool db)
{
        ...
        TabId >= purchased_tabs)
    {
        ...
        CharacterDatabase.PExecute("DELETE FROM guild_bank_right WHERE guildid='%u' AND rid='%u' AND TabId='%u'", Id, rankId, TabId);
        return;
    }
    if (rankId == GR_GUILDMASTER)
    {
...
void Guild::LogBankEvent(uint8 LogEntry, uint8 TabId, uint32 PlayerGuidLow, uint32 ItemOrMoney, uint8 ItemStackCount, uint8 DestTabId)
{
    ...
    if (NewEvent.isMoneyEvent())
    {
        ...
        {
            CharacterDatabase.PExecute("DELETE FROM guild_bank_eventlog WHERE guildid='%u' AND LogGuid='%u'", Id, m_GuildBankEventLog_Money.front().LogGuid);
    ...
    else
    {
        ...
        {
            CharacterDatabase.PExecute("DELETE FROM guild_bank_eventlog WHERE guildid='%u' AND LogGuid='%u'", Id, m_GuildBankEventLog_Item[TabId].front().LogGuid);
...
{
    QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT Min(LogGuid), Max(LogGuid) FROM guild_bank_eventlog WHERE guildid = %u", Id);
    if (!result)
        return;

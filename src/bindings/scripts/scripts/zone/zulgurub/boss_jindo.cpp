...
struct OREGON_DLL_DECL boss_jindoAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (Delusions_Timer < diff)
        {
            if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
            {
                ...
                Shade = m_creature->SummonCreature(14986, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 15000);
                if (Shade)
...
//Healing Ward
struct OREGON_DLL_DECL mob_healing_wardAI : public ScriptedAI
{
...
//Shade of Jindo
struct OREGON_DLL_DECL mob_shade_of_jindoAI : public ScriptedAI
{

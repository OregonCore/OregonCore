...
struct OREGON_DLL_DECL mobs_mana_tappedAI : public ScriptedAI
{
...
struct OREGON_DLL_DECL npc_prospector_anvilwardAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 i)
    {
        ...
        switch (i)
        {
            case 0: DoScriptText(SAY_PR_1, me, pPlayer); break;
            case 5: DoScriptText(SAY_PR_2, me, pPlayer); break;
...
struct OREGON_DLL_DECL npc_secondTrialAI : public ScriptedAI
{
...
struct OREGON_DLL_DECL master_kelerun_bloodmournAI : public ScriptedAI
{
      ...
      // fight the 4 paladin mobs phase
        ...
        if (timer <= diff) {
          ...
          paladinSpawn = (Unit::GetCreature((*me), paladinGuid[paladinPhase]));
            if (paladinSpawn) {
               ((npc_secondTrialAI*)paladinSpawn->AI())->Activate(me->GetGUID());
...
void npc_secondTrialAI::JustDied(Unit* Killer) {
      ...
      {
          Creature* Summoner;
          Summoner = (Unit::GetCreature((*me), summonerGuid));
...
bool GOHello_go_second_trial(Player *player, GameObject* _GO)
{
    ...
    CellPair p(Oregon::ComputeCellPair(_GO->GetPositionX(), _GO->GetPositionY()));
    Cell cell(p);
    ...
    Oregon::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*_GO, MASTER_KELERUN_BLOODMOURN, true, 30);
...
struct OREGON_DLL_DECL npc_apprentice_mirvedaAI : public ScriptedAI
{
...
struct OREGON_DLL_DECL npc_infused_crystalAI : public Scripted_NoMovementAI
{

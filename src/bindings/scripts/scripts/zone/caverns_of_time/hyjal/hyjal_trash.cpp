...
struct mob_frost_wyrmAI : public hyjal_trashAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (IsEvent || IsOverrun)
        {
            ((hyjal_trashAI*)me->AI())->SetCanAttack(false);
            npc_escortAI::UpdateAI(diff);
        }
        if (IsEvent)
        {
        ...
        if (!UpdateVictim())
            return;
        if (me->GetDistance(me->getVictim()) >= 25){
            if (MoveTimer <= diff)
            {
...
struct mob_gargoyleAI : public hyjal_trashAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!UpdateVictim())
            return;
        if (me->GetDistance(me->getVictim()) >= 20 || forcemove)
        {
            forcemove = false;
            if (forcemove)
            {
        ...
        if (StrikeTimer <= diff)
        {
            if (me->GetDistance(me->getVictim()) < 20)
            {
...
struct OREGON_DLL_DECL alliance_riflemanAI : public Scripted_NoMovementAI
{
